...
# ### Apply $z(L)$ to everything, nothing to $\tau$, predict the targets directly
# 
# # Think more about this - MIGHT HAVE TO INVERSE Z FIRST
...
# 
# $$
#     p_T^{\text{predicted}} = \mathbb{L}^{-1} \left[ z^{-1} (f_{\text{IQN}} ) \left[ \mathbb{L} (p_T^\text{gen})+10 \right] -10 \right] \qquad \text{if target is ratio}
# $$
# 
# 
# $$
#     p_T^{\text{predicted}} = \mathbb{L}^{-1} \left[ z^{-1} (f_{\text{IQN}} ) \right] \qquad \text{if target is not ratio}
# $$
# 
...
def split_t_x_RATIOTARGET(df, target, input_features):
...
starting_learning_rate=starting_learning_rate/100
BATCHSIZE=1000
...
traces_step = 50
n_iterations=10000
IQN = run(model=model,train_x=train_x, train_t=train_t, 
        valid_x=valid_x, valid_t=valid_t, traces=IQN_trace, n_batch=1000, 
...
p = pred.detach().numpy()
plt.hist(p, label='predicted target, scaled in some way');plt.legend();plt.show()
...
# 
# $$
#     m^{\text{predicted}} = \mathbb{L}^{-1} \left[ z^{-1} (f_{\text{IQN}} ) \left[ \mathbb{L} (m^\text{gen})+10 \right] -10 \right] \qquad \text{if target is ratio}
# $$
# 
# $$
# m^{\text{predicted}} = \mathbb{L}^{-1} \left[ z^{-1} (f_{\text{IQN}} ) \right] \qquad \text{if targr is not ratio}
# $$
# 
...
print(valid_t.shape, valid_t[:5])


# In[409]:


z_inv_f =z_inverse(xprime=p-5, mean=np.mean(valid_t+5), std=np.std(valid_t+5))
z_inv_f[:5]
...
plt.hist(z_inv_f, label=r'$z^{-1} f_{IQN}$',alpha=0.3);
...
# $$
# \text{"factor"}  = z^{-1} (f_{\text{IQN}} ) \left[ \mathbb{L} (m^\text{gen})+10 \right] -10 \qquad \text{if target is ratio}
# $$
# $$
# \text{"factor"}  = z^{-1} (f_{\text{IQN}} ) \qquad \text{if target is not ratio ratio}
# $$
...
plt.hist(m_pred.flatten());plt.show()
