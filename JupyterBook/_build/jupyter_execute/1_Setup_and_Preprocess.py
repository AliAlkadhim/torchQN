...
# # This jupyterbook is intended for my collaborators: Michelle, Braden, Harrison, Raghu. 
# 
...
# 
# ## Note that the binder will take a while to build, and you'd have to navigate through the files yourself (not as easy or nice as just going to https://alialkadhim.github.io/torchQN/). Furthermore, you'll have to download the dataset that I uploaded to Kaggle to replicate the results, which will also take long. I'm open to other suggestions.
# 
# ## If you want to run this interactively on binder, set `BINDER=True`. This will download our datasets using the Kaggle API. Our datasets are simply the following:Do `source setup.sh` before trying to run this notebook!

# ## 1.1: External Imports, Environment Setup and Binder
# 
...
# # 1.2: utils.py
# 
# This cell lists the utility functions and source code that I'll use later. Click [here](#1.3 Set arguments (optional) and configurations) To skip past this cell, as opposed to scrolling down!
...
# # 1.3: Set arguments (optional) and configurations
...
# # 1.4: Load and explore raw (unscaled) dataframes
# 
# ### use Joblib memory caching!
...
# $$
# 
# It is important to note that the z-descaling of the model $ z^{-1} (f_{\text{IQN}} )$ should be done using the *train set mean and standard deviation*, namely 
# 
# $$ z^{-1} (f_{\text{IQN}} ) =  z^{-1} \left[ f_{\text{IQN}}, \bar{x}=E[\mathbb{T}(\mathcal{O} ) ], \sigma = \sigma \left( \mathbb{T}( \mathcal{O}) \right) \right],$$ 
# 
# since we are predicting the ratios.
...
# ### How to de-scale Braden Scaling
