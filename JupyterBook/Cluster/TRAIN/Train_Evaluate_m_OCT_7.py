...
parser.add_argument('--n_iterations', type=int, help='The number of iterations for training, the default is', required=False,default=15000)
#default=5000000 )
parser.add_argument('--n_layers', type=int, help='The number of layers in your NN, the default is 5', required=False,default=12)
...
train_data, test_data, valid_data =load_raw_data()

@memory.cache
def old_split_t_x(df, target, source, scalers):
...
train_t, train_x =old_split_t_x(train_data, target, features, scalers)
valid_t, valid_x = old_split_t_x(valid_data, target, features, scalers)
test_t,  test_x  = old_split_t_x(test_data,  target, features, scalers)
...
def run(model, scalers, target, 
    ...
    return ModelHandler(model, scalers) 
...
def main():
    ...
    model =  RegularizedRegressionModel(nfeatures=train_x.shape[1], ntargets=1,nlayers=n_layers, hidden_size=n_hidden)
    traces = ([], [], [], [])
