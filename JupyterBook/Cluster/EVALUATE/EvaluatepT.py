...
# @memory.cache
def plot_one(
    target, real_edges, real_counts, predicted_counts, save_plot=False, PARAMS=None
):
    ...
    if save_plot:
        plot_filename = (
            utils.get_model_filename(target, PARAMS).split(".dict")[0] + ".png"
        )
        plt.savefig(
...
if __name__ == "__main__":
    ...
    if USE_BRADEN_SCALING == True:
        ...
        z_inv_f = z_inverse2(
            xprime=p, train_mean=np.mean(orig_ratio), train_std=np.std(orig_ratio)
        )
