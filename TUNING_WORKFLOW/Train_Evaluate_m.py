...
parser.add_argument('--n_iterations', type=int, help='The number of iterations for training, the default is', required=False,default=1000000)
#default=5000000 )
...
def run(model, scalers, target, 
    ...
    torch.save(model.state_dict(), 'trained_models/tuned_trained_mass.pt' )   
    ...
    # utils.plot_average_loss(traces)
...
def plot_model(dnn, target, src,
    ...
    ev_features=FIELDS[target]['inputs']
    ...
    new_cols= [target] + ev_features
    eval_data=eval_data.reindex(columns=new_cols)
    print('EVALUATION DATA NEW INDEX\n', eval_data.head())

    eval_data.to_csv('AUTOREGRESSIVE_m_Prime_%sk_iter.csv' % str(n_iterations/1000))
...
def main():
    ...
    traces = ([], [], [], [])
    dnn = run(model=model, scalers=scalers, target=target, train_x=train_x, train_t=train_t, valid_x=valid_x, valid_t=valid_t, traces=traces, 
              n_batch=batch_size, learning_rate=learning_rate)
    ...
    print('Total time for evaluating m: ',difference/3600, 'hours')
