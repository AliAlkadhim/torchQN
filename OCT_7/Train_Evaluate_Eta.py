...
def plot_model(dnn, target, src,
    ...
    eval_data=pd.read_csv('AUTOREGRESSIVE_m_Prime_pT_Prime.csv')
    
    old_cols =['RecoDatam', 'RecoDatapT'] +X 
    ...
    y = dnn(eval_data)
    eval_data['RecoDataeta']=y
    new_cols=['RecoDatam', 'RecoDatapT','RecoDataeta'] + X
    eval_data=eval_data.reindex(columns=new_cols)
    print('EVALUATION DATA NEW INDEX\n', eval_data.head())
    eval_data.to_csv('AUTOREGRESSIVE_m_Prime_pT_Prime_eta_Prime.csv')
    print('saved AUTOREGRESSIVE_m_Prime_pT_Prime_eta_Prime.csv')
...
def main():
    ...
    with open('eta_params.json','w') as f:
    ...
    print('evaluating eta took ',difference/3600, 'hours')
